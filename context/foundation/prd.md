...
- **No third-party integrations (Strava, Garmin, etc.)**: ride stats are entered by pasting plain text; no API syncing with external training platforms in v1.
...
No open questions — shape session completed with all elements present (quality_check_status: accepted, 2026-05-24).

1. **Auth implementation approach** — the FRs require user registration and login (FR-001–003); whether to build own auth or use a third-party provider is an open decision for `/10x-tech-stack-selector`. Owner: stack selection step. Block: no (does not affect PRD scope).
